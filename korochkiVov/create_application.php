...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $data = [
        ...
        'start_date' => clean_input($raw['start_date']),
        'payment_method' => clean_input($raw['payment_method']),
    ];
    with_old($data);
    $errors = validate_application($data, $raw);
    if (!$errors) {
        $displayDate = DateTime::createFromFormat('Y-m-d', $data['start_date'])->format('d.m.Y');
        $stmt = db()->prepare('INSERT INTO applications (user_id, course_id, desired_start_date, payment_method, status, created_at) VALUES (?, ?, ?, ?, ?, ?)');
        $stmt->execute([current_user()['id'], $data['course_id'], $displayDate, $data['payment_method'], 'new', date('Y-m-d H:i:s')]);
        clear_old();
...
<section class="section auth-shell">
  <div class="narrow compact-wrap">
    <div class="form-card compact-card reveal show">
      ...
      <h1>Подача заявки</h1>
      <p>Заполните короткую форму, чтобы передать заявку на рассмотрение и сохранить её в личном кабинете.</p>
      <form method="post" novalidate>
        ...
        <label>Желаемая дата начала
          <input type="date" name="start_date" value="<?= e(old('start_date')) ?>" min="<?= date('Y-m-d') ?>">
